package com.gordon.myblog.service;

import com.gordon.myblog.entity.ArticleCategoryRef;
import java.util.List;

/**
 * (ArticleCategoryRef)表服务接口
 *
 * @author makejava
 * @since 2020-05-10 15:43:26
 */
public interface ArticleCategoryRefService {

    /**
     * 通过ID查询单条数据
     *
     * @param articleId 主键
     * @return 实例对象
     */
    ArticleCategoryRef queryById(Integer articleId);

    /**
     * 查询多条数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<ArticleCategoryRef> queryAllByLimit(int offset, int limit);

    /**
     * 新增数据
     *
     * @param articleCategoryRef 实例对象
     * @return 实例对象
     */
    ArticleCategoryRef insert(ArticleCategoryRef articleCategoryRef);

    /**
     * 修改数据
     *
     * @param articleCategoryRef 实例对象
     * @return 实例对象
     */
    ArticleCategoryRef update(ArticleCategoryRef articleCategoryRef);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 是否成功
     */
    boolean deleteById(Integer id);

}