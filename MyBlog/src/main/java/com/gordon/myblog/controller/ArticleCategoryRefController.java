package com.gordon.myblog.controller;

import com.gordon.myblog.entity.ArticleCategoryRef;
import com.gordon.myblog.service.ArticleCategoryRefService;
import org.springframework.web.bind.annotation.*;

import javax.annotation.Resource;

/**
 * (ArticleCategoryRef)表控制层
 *
 * @author makejava
 * @since 2020-05-10 15:43:26
 */
@RestController
@RequestMapping("articleCategoryRef")
public class ArticleCategoryRefController {
    /**
     * 服务对象
     */
    @Resource
    private ArticleCategoryRefService articleCategoryRefService;

    /**
     * 通过主键查询单条数据
     *
     * @param id 主键
     * @return 单条数据
     */
    @GetMapping("selectOne")
    public ArticleCategoryRef selectOne(Integer id) {
        return this.articleCategoryRefService.queryById(id);
    }

}